...
> "단순 굿즈 쇼핑몰을 넘어서, **팬과 상호작용하고 보안까지 설계된 진짜 팬 플랫폼을 만들자**는 목표로 시작했습니다."

**HALO_SHOP**은 야구팬을 위한 굿즈 쇼핑몰에 그치지 않고,  
실시간 경매, 팬 출석 보상, 챗봇, 관리자 모니터링 시스템, 공격 감지 보안 미들웨어까지 통합한  
**전자상거래 + 팬 참여형 콘텐츠 + 보안 중심 설계의 커머스 플랫폼**입니다.

- **전자상거래** 기능(상품/결제/배송/리뷰)부터  
- **실시간 기능**(WebSocket 입찰/채팅, 알림/챗봇)  
- **보안 구조**(하이브리드 인증, 공격 탐지 미들웨어, 로그 시스템)  
- 그리고 **관리자 시스템**(권한/세션/로그 추적)까지,  
모든 기능이 하나의 통합된 흐름 안에서 작동하도록 설계했습니다.

> 특히 보안 중심의 백엔드 설계, 비동기 실시간 처리, 자동화된 배포 인프라를 통해  
> **실제 운영 가능한 커머스 시스템 수준의 완성도**를 목표로 개발하였습니다.
...
- **Infra**: AWS EC2, S3, Nginx, 배포 자동화 Shell 스크립트

---

### 📅 프로젝트 일정

- `2025.06.19 ~ 06.28` : **설계** (DB/ERD, Flow차트, Figma 화면 구성)  
- `2025.06.29 ~ 07.14` : **기능 구현 및 테스트** (API 개발, 프론트 연동, AWS 배포)
